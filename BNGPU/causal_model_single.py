...
class SingleCausalModel(nn.Module):
    ...
    def _init_B(self, B):
        ...
        self.B_tmp = nn.Parameter(copy.deepcopy(B), requires_grad=True)
        self.A_tmp = nn.Parameter(torch.randn([self.d * self.T, self.d]) * 0.01).to(B.device)
        # print(self.B.shape)
        self.B_tmp.data.fill_diagonal_(0)
        # print(self.B)
        # self.B = self.B)
    ...
    def _build(self):
        ...
        self.B = nn.Parameter(torch.randn([self.n, self.d, self.d]) * 0.01, requires_grad=False)
        # self.B = torch.randn([self.d, self.d]) * 0.01
    ...
    def _compute_likelihood(self, X, B):
        ...
        if self.equal_variances:    # Assuming equal noise variances
            ...
            return 0.5 * self.d * torch.log(
                torch.sum(
                        matrix_norm(X_t - X_t @ B - X_p@self.A_tmp)
                        # torch.square(X - X @ B), dim = (1, 2)
                    )
    ...
    # def (self, X, idx, labels = None):

    def forward(self, X, idx, labels = None):
        B = self.B_tmp
        # Time series; 
        # print(X.requires_grad, B.requires_grads)
        loss = self._compute_likelihood(X, B)
        penalty1 = self._compute_L1_penalty(B) + self._compute_L1_penalty(self.A_tmp)
        penalty2 = self._compute_h(B)

        # print(loss, penalty1, penalty2)
